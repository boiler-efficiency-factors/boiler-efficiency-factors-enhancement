...
function Frame2({ workspaceName, onInfoClick }: { workspaceName: string; onInfoClick: () => void }) {
  return (
    <div className="content-stretch flex h-[37px] items-center justify-between overflow-clip px-0 py-[4px] relative shrink-0 w-full">
      <p className="font-['Noto_Sans_KR:Bold',sans-serif] font-bold leading-[20px] relative shrink-0 text-[24px] text-[color:var(--darkreader-text-243647,#2c4257)] text-nowrap whitespace-pre">{`워크스페이스명 : ${workspaceName}`}</p>
...
function Frame1({ workspaceName, onInfoClick }: { workspaceName: string; onInfoClick: () => void }) {
  return (
...
function AuthCreateAccount({ workspaceName, onInfoClick }: { workspaceName: string; onInfoClick: () => void }) {
  return (
    <div className="absolute bg-[var(--darkreader-background-f9f9f9,#d8d6d2)] content-stretch flex flex-col items-center left-0 overflow-clip px-[100px] py-[60px] top-[72px] w-[1440px]" data-name="Auth/Create-Account">
      <Frame1 workspaceName={workspaceName} onInfoClick={onInfoClick} />
    </div>
  );
}
...
function NavBar({ username, onBack }: { username: string; onBack: () => void }) {
  return (
    <div className="absolute contents left-0 top-0" data-name="Nav-bar">
      ...
      <button
        onClick={onBack}
        className="absolute left-[20px] top-[26px] cursor-pointer hover:opacity-70 transition-opacity"
      >
